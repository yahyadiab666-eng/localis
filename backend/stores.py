...
from backend.image_batch import DEFAULT_IMAGEN, IMAGEN_PENDIENTE, encolar_procesamiento_imagenes
    ...
    try:
        ...
        with get_db_connection(row_factory=sqlite3.Row) as conexion:
            ...
            for fila in filas:
                ...
                d['imagen_completa'] = _normalizar_imagen_url(d.get('imagen_url'))
                productos.append(d)
...
def _parsear_fila_producto(fila):
    ...
    return {
        ...
        'imagen_url': str(fila.get('imagen_url') or '').strip(),
    }


def procesar_csv_productos(comercio_id, archivo_csv, upload_folder=None):
    """
    Carga masiva con reemplazo total. Inserta referencias rápido en SQLite
    y procesa compresión/búsqueda de imágenes en paralelo en segundo plano.
    """
    ...
    try:
        ...
        if upload_folder and productos_pendientes:
            encolar_procesamiento_imagenes(
                comercio_id, productos_pendientes, upload_folder
            )

        return (
            True,
            f'Carga completada: {insertados} productos importados. '
            f'Las imágenes se están procesando en segundo plano.',
        )
