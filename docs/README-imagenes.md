...
- **Anti-bloqueo (`backend/http_client.py`):** rotación de User-Agents de navegadores reales, cabeceras completas (`Accept-Language`, `Sec-Fetch-*`…), reintentos con **backoff exponencial** y soporte de **proxy HTTP(S)** (`LOCALIS_HTTP_PROXY`) para saltar bloqueos de IP del datacenter.
