...
def _auditar_importacion_instantanea():
    ...
    _ok('def _buscar_brave_html' in pipeline_src, 'motor secundario HTML (Brave Search)')
    _ok('def logo_wikidata' in _leer('backend/marca_logo.py'), 'logo oficial vectorial (Wikidata/Commons)')
    _ok('def _proxies_publicos' in _leer('backend/http_client.py'), 'rotación de proxies (anti-bloqueo)')
